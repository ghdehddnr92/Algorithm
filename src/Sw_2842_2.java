...
//							map[i][j].flag=1;
//							if(map[i][j].dir==1){
//								if(map[i-1][j]==null){ // 이동해야할 곳이 비어있는지 확인.  비어 있을 경우
...
//											map[i-1][j].num+=map[i-1][j].miTmp[c][0];// 동시에 들어오는 미생물을 모두 합치는 소스.
...
//												map[i-1][j].dir=map[i-1][j].miTmp[c][1];
//											}
//										}
//									}
//								}
...
//										map[i][j-1].num+=map[i][j-1].miTmp[c][0];// 동시에 들어오는 미생물을 모두 합치는 소스.
...
//											map[i][j-1].dir=map[i][j-1].miTmp[c][1];
//										}
//									}
//								}
//
